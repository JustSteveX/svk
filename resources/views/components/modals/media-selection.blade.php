<x-modal focusable name="mediaSelection" title="Medien-Auswahl">
		...
		<div class="grid grid-cols-4 gap-2">
              ...
              <div class="flex items-center justify-center w-full h-full" @click="selected = !selected; selectedData = selected ? [...selectedData, {'name': '{{ $mediaItem->name }}', 'mime_type': '{{$mediaItem->mime_type}}' }] : selectedData.filter(item => item.name !== '{{ $mediaItem->name }}')">
