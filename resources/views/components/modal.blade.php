    ...
    if (value) {
...
})" x-on:close-all-modals.window="show = false" x-on:close.stop="show = false"
		x-on:keydown.escape.window="show = false" x-on:keydown.shift.tab.prevent="prevFocusable().focus()"
		x-on:keydown.tab.prevent="$event.shiftKey || nextFocusable().focus()"
		x-on:open-modal.window="$event.detail == '{{ $name }}' ? show = true : null" x-show="show">
		<div class="fixed inset-0 transition-all transform" x-on:click="show = false" x-show="show"
		...
		<div
				...
				<div class="flex flex-row justify-between p-4">
						...
						<button type="button" class="text-gray-600 w-7 h-7 hover:text-gray-400"
								x-on:click="show = false"><x-eos-close /></button>
